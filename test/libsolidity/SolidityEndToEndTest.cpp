...
BOOST_AUTO_TEST_CASE(fixed_type_incrementing_decrementing)
{
	char const* sourceCode = R"(
		contract C {
			...
			function daWhile() returns (ufixed8x8) {
				var i = 3.25;
				while ( i < ufixed8x8(500/1)) {
					i++;
				}
				return i;
			}
			function daFor() returns (ufixed8x8) {
				for (var i = 3.25; i < ufixed8x8(500/1); i++) {
					var j = i;
				}
				return i;
			}
	...
	BOOST_CHECK(callContractFunction("daWhile()") == encodeArgs(fixed(977,4,8)));
	BOOST_CHECK(callContractFunction("daFor()") == encodeArgs(fixed(977,4,8)));
}
